...
var ajax = new XMLHttpRequest(); //const=상수 : 값을 변경할 일 없을때 씀
...
var newsFeed = JSON.parse(ajax.response); // JSON.parse : 괄호안의 JSON 데이터 값을 객체로 바꾼다 == 데이터를 Javascript에서 다룰수 있는 형태로 바꿈

var ul = document.createElement('ul'); // createElement : 태그를 만든다 // = ul태그를만든다

for (var i = 0; i < 10; i++) {
  document.getElementById('root').innerHTML = "<ul>\n      <li>".concat(newsFeed[i].title, "</li>\n      <li>").concat(newsFeed[i].title, "</li>\n      <li>").concat(newsFeed[2].title, "</li>\n    </ul>");
} //반복문 : for
//백틱(``) : 문자열을 만드는것 // ul : 순서가 필요없는 목록 생성 // li : 목록에 포함된 아이템
